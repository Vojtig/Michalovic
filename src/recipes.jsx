...
function App() {
  ...
  const saveRecipe = async recipe => {
    ...
    try {
      const res = await fetch(API_URL + '?_=' + Date.now(), { headers: { 'X-Token': API_TOKEN }, cache: 'no-store' });
      if (res.ok) { const data = await res.json(); const fresh = normalizeRecipes(data); if (fresh) baseRecipes = fresh; }
    } catch {}
